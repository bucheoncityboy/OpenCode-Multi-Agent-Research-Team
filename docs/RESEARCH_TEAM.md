...
#### 📄 STRATEGY_REPORT.md 양식

```markdown
# {전략명} 전략 보고서

## 1. 핵심 로직 (Core Logic)
- **핵심 알파**: (예: 볼린저 밴드 하단 터치 시 평균 회귀)
- **진입 조건**: 구체적 수식/조건 명시
- **청산 조건**: 구체적 수식/조건 명시
- **필터 (있는 경우)**: 레짐, 변동성 등
- **수학적 근거 / Inefficiency**: 왜 이 로직이 작동하는지 설명
...
### 3-1. 방법론
- **인샘플 기간**: YYYY-MM-DD ~ YYYY-MM-DD
- **아웃샘플 기간**: YYYY-MM-DD ~ YYYY-MM-DD
- **거래비용**: X bps
- **슬리피지**: X bps
...
| 검증 | 결과 | 통과 |
|------|------|------|
| Monte Carlo p-value (1000회 셔플) | p = X.XX | ☐ |
| Bootstrap 95% CI (Sharpe) | [X.XX, X.XX] | ☐ |
| Bootstrap 95% CI (수익률) | [X.XX, X.XX] | ☐ |

### 3-5. 크로스 자산 검증 결과
| 자산 | OOS Sharpe | OOS MDD | 통과 |
|------|------------|---------|------|
| {메인 자산} | | | ☐ |
| {추가 자산 1} | | | ☐ |
| {추가 자산 2} | | | ☐ |
...
### 2-2. 성과 비교
| ID | 전략명 | OOS Sharpe | OOS MDD | 거래 횟수 | OOS 기간 |
|----|--------|------------|---------|-----------|----------|
| 01 | | | | | |
| 02 | | | | | |
| ... | | | | | |
...
| 카테고리 | 전략 | 설명 |
|----------|------|------|
| 모멘텀/추세 | S01, S05 | 추세 추종 계열 |
| 평균 회귀 | S02, S07 | 과매수/과매도 역추세 |
| 차익거래 | S03 | 스프레드, 현선물 등 |
| 캐리/펀딩 | S04 | 펀딩비, 김프 등 |
| 레짐 기반 | S06 | 시장 상태 필터 활용 |
| 기타 | | |
...
---
**Participating Agents**: @research-lead, @research-coder, @research-analyst, ...
```
...
### 2-4. 백테스팅 표준 환경 (Standardized Backtest Environment)

모든 전략은 **동일한 보수적 표준 환경**에서 최종 검증되어야 한다.
...
| 슬리피지 | 편도 3bps | 진입/청산 각각 적용 |
| 거래 비용 | 편도 4bps | 진입/청산 각각 적용 |
| 데이터 | Binance OHLCV | - |

> **총 비용 예시**: 롱 진입 → 롱 청산 = (3+4) × 2 = **14bps**

#### 필수 원칙
1. **모든 전략** = 시장가로 통일 검증
2. **지정가 전략**도 시장가 환경에서 재검증 필수
3. **표준 환경 미통과** = 알파 풀 등록 불가

#### 카테고리별 비교 원칙
| 비교 유형 | 방법 |
|-----------|------|
| 같은 카테고리 내 | Sharpe, MDD 직접 비교 |
| 다른 카테고리 간 | 상관관계로 포트폴리오 조합 판단 |

> ⚠️ **금지**: 모멘텀 vs 차익거래 Sharpe 직접 비교 (성질이 다름)
...
| 카테고리 | 설명 | 예시 |
|----------|------|------|
| **Market Microstructure** | 호가창, 체결 패턴 | Order flow imbalance, VPIN 등등|
| **Cross-Asset** | 자산간 상관관계 | BTC-ETH spread, Equity-Bond rotation 등등|
| **Volatility Regime** | 변동성 레짐 전환 | VIX-based, GARCH regime 등등|
| **Seasonality** | 시간대/요일/월별 패턴 | 아시아 세션 모멘텀, 월말 리밸런싱 등등|
| **Liquidity Premium** | 유동성 기반 | Illiquidity premium, Volume anomaly 등등|
| **Sentiment/Flow** | 자금 흐름 | Funding rate, Open Interest 등등|
| **Statistical Arbitrage** | 통계적 비효율 | Mean reversion, Pairs trading 등등|
...
### 7-2. [CRITICAL] 로직 다양성 원칙 (Logic Diversity Rule)

동일한 핵심 로직의 변형을 여러 개 만드는 것을 **엄격히 금지**한다.

#### 핵심 로직 정의
- **핵심 로직**: 수익을 만드는 주요 신호 (예: 볼린저 터치, EMA 크로스, 모멘텀)
- **변형**: 파라미터 변경, 필터 추가/변경은 핵심 로직이 같으면 동일 취급

#### 규칙
1. **같은 핵심 로직 = 최적 파라미터 1개만** 알파 풀에 등록
2. **요청 수량 = 서로 다른 핵심 로직 수량**
   - `/rt mine crypto 10` → 10개 모두 다른 핵심 로직

#### 예시
| 케이스 | 판정 |
|--------|------|
| EMA 5/60, EMA 10/100, EMA 20/200 각각 등록 | ❌ 금지 (파라미터 변형) |
| 볼린저+RSI, 볼린저+ADX 각각 등록 | ❌ 금지 (필터 변형) |
| EMA 크로스 1개 + 볼린저 역추세 1개 | ✅ 허용 (다른 핵심 로직) |

### 7-3. [CRITICAL] 크로스 자산 검증 (Cross-Asset Validation)

단일 자산에서만 작동하는 알파는 **과적합으로 간주**하여 기각한다.

#### 규칙
1. 알파는 **최소 3개 이상 자산**에서 유효해야 함
2. 메인 자산 + 최소 2개 추가 자산에서 OOS 검증 통과 필수
3. 단일 자산 전용 알파 → 즉시 기각

#### 예시
| 케이스 | 판정 |
|--------|------|
| BTC에서만 Sharpe 2.0, ETH/SOL 음수 | ❌ 기각 (과적합) |
| BTC Sharpe 1.8, ETH 1.5, SOL 1.2 | ✅ 허용 |

#### 예외
- 자산 고유 특성 기반 전략 (예: 김프, 특정 거래소 차익)은 예외 허용
- 단, STRATEGY_REPORT에 "단일 자산 전용" 및 사유 명시 필수
